...
type ConfigCustom struct {
	...
	countExhausted    bool                // Trigger to indicate termination.
	Delimiter         string              // Delimiter between path elements when converted to string.
	Display           func(proto.Message) // Function called to display each result.
	DisplayPrefix     string              // Prefix for each line of result output.
	DisplayIndent     string              // Indent per nesting level of result output.
	DisplayType       string              // Display results in selected format, grouped, single, proto.
	DisplayPeer       bool                // Display the immediate connected peer.
...
// Custom QueryDisplay
func QueryDisplayCustom(ctx context.Context, query client.Query, cfg *ConfigCustom) error {
	if err := sendQueryAndDisplayCustom(ctx, query, cfg); err != nil {
		return fmt.Errorf("sendQueryAndDisplay(ctx, %+v, %+v):\n\t%v", query, cfg, err)
	}
	return nil
}
...
/// Custom sendQueryAndDisplay

func sendQueryAndDisplayCustom(ctx context.Context, query client.Query, cfg *ConfigCustom) error {
	cancel := func() {}
	if cfg.StreamingDuration > 0 {
		ctx, cancel = context.WithTimeout(ctx, cfg.StreamingDuration)
		defer cancel()
	...
	default:
		return fmt.Errorf("unknown display type %q", cfg.DisplayType)
	case "RAW":
		return displayProtoResultsRaw(ctx, query, cfg, func(r proto.Message) proto.Message {
			return r
		})
	...
	default:
		return fmt.Errorf("unknown query type %v", query.Type)
	case client.Stream:
		return displayStreamingResultsCustom(ctx, query, cfg)
	}
}
...
func displayProtoResultsRaw(ctx context.Context, query client.Query, cfg *ConfigCustom, formatter func(proto.Message) proto.Message) error {
	var sum int64
	query.ProtoHandler = func(msg proto.Message) error {
		if cfg.DisplaySize {
			sum += int64(proto.Size(msg))
...
// Custom Display Streaming results

func displayStreamingResultsCustom(ctx context.Context, query client.Query, cfg *ConfigCustom) error {
	c := client.New()
	complete := false
	display := func(path []string, ts time.Time, val interface{}) {
		if !complete {
	...
	query.NotificationHandler = func(n client.Notification) error {
		...
		case client.Delete:
			display(v.Path, v.TS, v.Val)
		case client.Sync:
			displayWalkCustom(query.Target, c, cfg)
			complete = true
			// case client.Error:
			// 	cfg.Display([]byte(fmt.Sprintf("Error: %v", v)))
		}
...
// Custom displaLog
func displayWalkCustom(target string, c *client.CacheClient, cfg *ConfigCustom) {
	b := make(pathmap)
	var addFunc func(path []string, v client.TreeVal)
	switch cfg.Timestamp {
	default:
